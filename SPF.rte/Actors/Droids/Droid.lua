...
function Update(self)
	...
	if iniPainOrDeath then
		self.PainSound.Pos = self.Pos; -- they get long and CC doesn't do it for us
		...
		if self.spotAllowed ~= false then
			...
			if self.LastTargetID == -1 then
				...
				then
					...
					if (not iniPainOrDeath) and not self.voiceSound:IsBeingPlayed() and math.random(0, 100) < 60 then
