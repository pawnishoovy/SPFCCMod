function Create(self)
	...
	self.preSound = CreateSoundContainer("Pre PTR-Z 55", "SPF.rte");
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors PTR-Z 55", "SPF.rte");
	
	self.reloadPrepareSounds = {}
	self.reloadPrepareSounds.BoltBack = nil
	self.reloadPrepareSounds.MagOut = CreateSoundContainer("MagOutPrepare PTR-Z 55", "SPF.rte");
	self.reloadPrepareSounds.MagIn = CreateSoundContainer("MagInPrepare PTR-Z 55", "SPF.rte");
	self.reloadPrepareSounds.BoltForward = nil
	
	self.reloadPrepareLengths = {}
	self.reloadPrepareLengths.BoltBack = 0
	self.reloadPrepareLengths.MagOut = 1090
	self.reloadPrepareLengths.MagIn = 810
	self.reloadPrepareLengths.BoltForward = 0
	
	self.reloadPrepareDelay = {}
	self.reloadPrepareDelay.BoltBack = 300
	self.reloadPrepareDelay.MagOut = 1090
	self.reloadPrepareDelay.MagIn = 1150
	self.reloadPrepareDelay.BoltForward = 150
	
	self.reloadAfterSounds = {}
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack PTR-Z 55", "SPF.rte");
	self.reloadAfterSounds.MagOut = CreateSoundContainer("MagOut PTR-Z 55", "SPF.rte");
	self.reloadAfterSounds.MagIn = CreateSoundContainer("MagIn PTR-Z 55", "SPF.rte");
	self.reloadAfterSounds.BoltForward = CreateSoundContainer("BoltForward PTR-Z 55", "SPF.rte");
	
	self.reloadAfterDelay = {}
	self.reloadAfterDelay.BoltBack = 175
	self.reloadAfterDelay.MagOut = 1150
	...
	self.recoilDamping = 0.2
	
	self.recoilMax = 25 -- in deg.
	self.originalSharpLength = self.SharpLength
	-- Progressive Recoil System 
end
...
function Update(self)
		...
		if self.reloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			if self.afterSoundPlayed ~= true then
				...
				if self.reloadPhase == 0 then
					self.phaseOnStop = 1;
					local fake
					fake = CreateMOSRotating("Fake Magazine MOSRotating PTR-Z 55");
					fake.Pos = self.Pos + Vector(-1 * self.FlipFactor, 2):RadRotate(self.RotAngle);
	...
	if self.FiredFrame then
		...
		if self.parent and self.parent:IsPlayerControlled() then
			...
			self.RateOfFire = 200; -- let players spam!!
			...
			self.RateOfFire = 50; -- DON'T let AI uselessly spam!!!
