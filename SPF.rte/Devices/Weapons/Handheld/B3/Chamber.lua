function Create(self)
	...
	self.preSound = CreateSoundContainer("Pre B-3", "SPF.rte");
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors B-3", "SPF.rte");
	
	self.reloadPrepareSounds = {}
	self.reloadPrepareSounds.MagOut = CreateSoundContainer("MagOutPrepare B-3", "SPF.rte");
	self.reloadPrepareSounds.MagIn = CreateSoundContainer("MagInPrepare B-3", "SPF.rte");
	self.reloadPrepareSounds.BoltBack = CreateSoundContainer("BoltBackPrepare B-3", "SPF.rte");
	self.reloadPrepareSounds.BoltForward = nil;
	
	self.reloadPrepareLengths = {}
	self.reloadPrepareLengths.MagOut = 1620
	self.reloadPrepareLengths.MagIn = 910
	self.reloadPrepareLengths.BoltBack = 120
	self.reloadPrepareLengths.BoltForward = 0
	
	self.reloadPrepareDelay = {}
	self.reloadPrepareDelay.MagOut = 1620
	self.reloadPrepareDelay.MagIn = 2860
	self.reloadPrepareDelay.BoltBack = 300
	self.reloadPrepareDelay.BoltForward = 215
	
	self.reloadAfterSounds = {}
	self.reloadAfterSounds.MagOut = CreateSoundContainer("MagOut B-3", "SPF.rte");
	self.reloadAfterSounds.MagIn = CreateSoundContainer("MagIn B-3", "SPF.rte");
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack B-3", "SPF.rte");
	self.reloadAfterSounds.BoltForward = CreateSoundContainer("BoltForward B-3", "SPF.rte");
	
	self.reloadAfterDelay = {}
	self.reloadAfterDelay.MagOut = 1000
	self.reloadAfterDelay.MagIn = 300
	self.reloadAfterDelay.BoltBack = 200
	self.reloadAfterDelay.BoltForward = 250
	...
	self.recoilStrength = 16 -- multiplier for base recoil added to the self.recoilStr when firing
...
function Update(self)
	...
	if self:IsReloading() then
		...
		if self.reloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			if self.afterSoundPlayed ~= true then
				...
				if self.reloadPhase == 0 then
					self.phaseOnStop = 1;
					local fake
					fake = CreateMOSRotating("Fake Magazine MOSRotating B-3");
					fake.Pos = self.Pos + Vector(-8 * self.FlipFactor, 5):RadRotate(self.RotAngle);
		...
		self.Frame = math.floor(f * 4 + 0.55);
	end
	

	if self:DoneReloading() and self.chamberOnReload then
		self.Magazine.RoundCount = 120;
		self.chamberOnReload = false;
	elseif self:DoneReloading() then
		self.Magazine.RoundCount = 121;
		self.chamberOnReload = false;
	end	
	...
	if self.FiredFrame then
		...
		self.horizontalAnim = 26
	
		self.FireTimer:Reset();
	
		self.angVel = self.angVel - RangeRand(0.7,1.1) * 2
