function Create(self)
	...
	self.preSound = CreateSoundContainer("Pre AutoMag D-9", "SPF.rte");
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors AutoMag D-9", "SPF.rte");
	
	self.reloadPrepareSounds = {}
	self.reloadPrepareSounds.MagOut = CreateSoundContainer("MagOutPrepare AutoMag D-9", "SPF.rte");
	self.reloadPrepareSounds.MagIn = CreateSoundContainer("MagInPrepare AutoMag D-9", "SPF.rte");
	self.reloadPrepareSounds.BoltBack = nil;
	self.reloadPrepareSounds.BoltForward = nil;
	
	self.reloadPrepareLengths = {}
	self.reloadPrepareLengths.MagOut = 920
	self.reloadPrepareLengths.MagIn = 770
	self.reloadPrepareLengths.BoltBack = 0
	self.reloadPrepareLengths.BoltForward = 0
	
	self.reloadPrepareDelay = {}
	self.reloadPrepareDelay.MagOut = 920
	self.reloadPrepareDelay.MagIn = 2000
	self.reloadPrepareDelay.BoltBack = 300
	self.reloadPrepareDelay.BoltForward = 185
	
	self.reloadAfterSounds = {}
	self.reloadAfterSounds.MagOut = CreateSoundContainer("MagOut AutoMag D-9", "SPF.rte");
	self.reloadAfterSounds.MagIn = CreateSoundContainer("MagIn AutoMag D-9", "SPF.rte");
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack AutoMag D-9", "SPF.rte");
	self.reloadAfterSounds.BoltForward = CreateSoundContainer("BoltForward AutoMag D-9", "SPF.rte");
	
	self.reloadAfterDelay = {}
	self.reloadAfterDelay.MagOut = 600
	self.reloadAfterDelay.MagIn = 100
	self.reloadAfterDelay.BoltBack = 185
		...
		if self.reloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			if self.afterSoundPlayed ~= true then
				...
				if self.reloadPhase == 0 then
					self.phaseOnStop = 1;
					local fake
					fake = CreateMOSRotating("Fake Magazine MOSRotating AutoMag D-9");
					fake.Pos = self.Pos + Vector(1 * self.FlipFactor, -3):RadRotate(self.RotAngle);
