function Create(self)
	...
	self.preSound = CreateSoundContainer("Pre MKII Harksburg Flarepistol", "SPF.rte");
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors MKII Harksburg Flarepistol", "SPF.rte");
	
	self.reloadPrepareSounds = {}
	self.reloadPrepareSounds.Open = CreateSoundContainer("OpenPrepare MKII Harksburg Flarepistol", "SPF.rte");
	self.reloadPrepareSounds.InsertShell = CreateSoundContainer("InsertShellPrepare MKII Harksburg Flarepistol", "SPF.rte");
	self.reloadPrepareSounds.Close = CreateSoundContainer("ClosePrepare MKII Harksburg Flarepistol", "SPF.rte");
	self.reloadPrepareSounds.HammerBack = nil;
	
	self.reloadPrepareLengths = {}
	self.reloadPrepareLengths.Open = 155
	self.reloadPrepareLengths.InsertShell = 440
	self.reloadPrepareLengths.Close = 95
	self.reloadPrepareLengths.HammerBack = 0
	
	self.reloadPrepareDelay = {}
	self.reloadPrepareDelay.Open = 155
	self.reloadPrepareDelay.InsertShell = 900
	self.reloadPrepareDelay.Close = 300
	self.reloadPrepareDelay.HammerBack = 250
	
	self.reloadAfterSounds = {}
	self.reloadAfterSounds.Open = CreateSoundContainer("Open MKII Harksburg Flarepistol", "SPF.rte");
	self.reloadAfterSounds.InsertShell = CreateSoundContainer("InsertShell MKII Harksburg Flarepistol", "SPF.rte");
	self.reloadAfterSounds.Close = CreateSoundContainer("Close MKII Harksburg Flarepistol", "SPF.rte");
	self.reloadAfterSounds.HammerBack = CreateSoundContainer("HammerBack MKII Harksburg Flarepistol", "SPF.rte");
	
	self.reloadAfterDelay = {}
	self.reloadAfterDelay.Open = 300
	self.reloadAfterDelay.InsertShell = 250
	self.reloadAfterDelay.Close = 250
	self.reloadAfterDelay.HammerBack = 300
...
function Update(self)
	...
	if self.parent and self:IsReloading() then
	
		if self.Frame == 0 and self.reloadPhase == 0 then
			self.Frame = 1;
		...
		if self.reloadPhase == 0 then
			...
			self.parent.HolsterOffset = Vector(-15, -15)
		...
		elseif self.reloadPhase == 1 then
			...
			self.reloadDelay = self.reloadPrepareDelay.InsertShell;
			self.afterDelay = self.reloadAfterDelay.InsertShell;		
			
			self.prepareSound = self.reloadPrepareSounds.InsertShell;
			self.prepareSoundLength = self.reloadPrepareLengths.InsertShell;
			self.afterSound = self.reloadAfterSounds.InsertShell;
			--
		...
		elseif self.reloadPhase == 2 then
			...
			--
			self.rotationTarget = 2 * math.pow(self.reloadTimer.ElapsedSimTimeMS / (self.reloadDelay + self.afterDelay), 3)
		...
		if self.reloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			if self.afterSoundPlayed ~= true then
				...
				if self.reloadPhase == 0 then
					...
					if not self.spentRound then
						if self:NumberValueExists("Switched") then
							if self:NumberValueExists("Frag Round") then
								local shell = CreateMOSRotating("MKII Harksburg Flarepistol Inert Flare A");
								shell.Pos = self.Pos;
								shell.Vel = Vector(math.random(-4, -2)*self.FlipFactor, 0):RadRotate(self.RotAngle);
								MovableMan:AddParticle(shell);
							else
								local shell = CreateMOSRotating("MKII Harksburg Flarepistol Inert Flare B");
								shell.Pos = self.Pos;
								shell.Vel = Vector(math.random(-4, -2)*self.FlipFactor, 0):RadRotate(self.RotAngle);
								MovableMan:AddParticle(shell);						
							end
	...
	if self.canSmoke and not self.FireTimer:IsPastSimMS(500) then
