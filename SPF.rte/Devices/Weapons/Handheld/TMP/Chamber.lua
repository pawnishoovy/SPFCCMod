...
function Create(self)
	...
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors TMP", "SPF.rte");
	
	self.reloadPrepareSounds = {}
	self.reloadPrepareSounds.MagOut = CreateSoundContainer("MagOutPrepare TMP", "SPF.rte");
	self.reloadPrepareSounds.MagIn = CreateSoundContainer("MagInPrepare TMP", "SPF.rte");
	self.reloadPrepareSounds.BoltBack = nil;
	self.reloadPrepareSounds.BoltForward = nil;
	
	self.reloadPrepareLengths = {}
	self.reloadPrepareLengths.MagOut = 90
	...
	self.reloadPrepareDelay.BoltForward = 80
	
	self.reloadAfterSounds = {}
	self.reloadAfterSounds.MagOut = CreateSoundContainer("MagOut TMP", "SPF.rte");
	self.reloadAfterSounds.MagIn = CreateSoundContainer("MagIn TMP", "SPF.rte");
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack TMP", "SPF.rte");
	self.reloadAfterSounds.BoltForward = CreateSoundContainer("BoltForward TMP", "SPF.rte");
	
	self.reloadAfterDelay = {}
	self.reloadAfterDelay.MagOut = 550
	self.reloadAfterDelay.MagIn = 250
	self.reloadAfterDelay.BoltBack = 80
...
function Update(self)
	...
	if self:IsReloading() then
		...
		if self.reloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			if self.afterSoundPlayed ~= true then
				...
				if self.reloadPhase == 0 then
					self.phaseOnStop = 1;
					local fake
					fake = CreateMOSRotating("Fake Magazine MOSRotating TMP");
					fake.Pos = self.Pos + Vector(0 * self.FlipFactor, 2):RadRotate(self.RotAngle);
		...
		local f = math.max(1 - math.min((self.FireTimer.ElapsedSimTimeMS) / 80, 1), 0)
