function Create(self)
	...
	self.preSound = CreateSoundContainer("Pre ARk Riot", "SPF.rte");
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors ARk Riot", "SPF.rte");
	...
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack ARk Riot", "SPF.rte");
	self.reloadAfterSounds.RoundIn = CreateSoundContainer("RoundIn ARk Riot", "SPF.rte");
	self.reloadAfterSounds.BoltForward = CreateSoundContainer("BoltForward ARk Riot", "SPF.rte");
	
	self.reloadAfterDelay = {}
	self.reloadAfterDelay.BoltBack = 100
	self.reloadAfterDelay.RoundIn = 200
	self.reloadAfterDelay.BoltForward = 120
	...
	self.recoilDamping = 0.35
	
	self.recoilMax = 17 -- in deg.
	self.originalSharpLength = self.SharpLength
	-- Progressive Recoil System 
end
	...
	if self.parent and (self:IsReloading() or self.Chamber == true) then
		...
		if self.reloadPhase == 0 then
			...
			if self.spentRound then
				self.reloadDelay = self.reloadPrepareDelay.RoundIn * 3;
				self.afterDelay = self.reloadAfterDelay.RoundIn * 3;
			end			
			...
			--
			self.rotationTarget = 4 * (math.pow(self.reloadTimer.ElapsedSimTimeMS / (self.reloadDelay + self.afterDelay), 3) - 0.5) * 2			
		...
		elseif self.reloadPhase == 2 then
			...
			if self:IsReloading() then
				self.reloadDelay = self.reloadPrepareDelay.BoltForward * 1.5;
			end
		...
		if self.reloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			elseif self.reloadPhase == 1 then
				...
				self.Frame = math.floor(factor * (5) + 0.5)
			...
			elseif self.reloadPhase == 2 then
				...
				self.Frame = math.floor((1 - factor) * (5) + 0.5)
				
				self.rotationTarget = -6 - -10 * factor
			end
			...
			if self.reloadTimer:IsPastSimMS(self.reloadDelay + self.afterDelay) then
				...
				if self.reloadPhase == 0 then	
					...
					elseif self.ammoCount == 7 or self.breakReload == true then
						self.Chamber = false;
	...
	if self.FiredFrame then
		...
		for i = 1, 6 do
			...
			if Effect then
				...
				Effect.Vel = (self.Vel + Vector(RangeRand(-20,20), RangeRand(-30, 30)) + Vector(150*self.FlipFactor,0):RadRotate(self.RotAngle)) / 30
