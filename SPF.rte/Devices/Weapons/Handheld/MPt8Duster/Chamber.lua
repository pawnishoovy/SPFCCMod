function Create(self)
	...
	self.boltSound = CreateSoundContainer("BoltSound MPt-8 Duster", "SPF.rte");
	self.boltDropSound = CreateSoundContainer("BoltDrop MPt-8 Duster", "SPF.rte");
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors MPt-8 Duster", "SPF.rte");
	
	self.reloadPrepareSounds = {}
	self.reloadPrepareSounds.BoltBack = nil
	self.reloadPrepareSounds.MagOut = nil
	self.reloadPrepareSounds.MagIn = CreateSoundContainer("MagInPrepare MPt-8 Duster", "SPF.rte");
	
	self.reloadPrepareLengths = {}
	self.reloadPrepareLengths.BoltBack = 0
	self.reloadPrepareLengths.MagOut = 0
	self.reloadPrepareLengths.MagIn = 350
	
	self.reloadPrepareDelay = {}
	self.reloadPrepareDelay.BoltBack = 300
	self.reloadPrepareDelay.MagOut = 200
	self.reloadPrepareDelay.MagIn = 900
	
	self.reloadAfterSounds = {}
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack MPt-8 Duster", "SPF.rte");
	self.reloadAfterSounds.MagOut = CreateSoundContainer("MagOut MPt-8 Duster", "SPF.rte");
	self.reloadAfterSounds.MagIn = CreateSoundContainer("MagIn MPt-8 Duster", "SPF.rte");
...
function Update(self)
	...
	if self:IsReloading() then
		...
		if self.reloadTimer:IsPastSimMS(self.reloadDelay) then
			...
			if self.reloadPhase == 2 then
				...
				self.Frame = 0 +  math.floor((factor) * (2) + 0.5)
			...
			if self.afterSoundPlayed ~= true then
				...
				if self.reloadPhase == 0 then
					...
					local fake
					fake = CreateMOSRotating("Fake Magazine MOSRotating MPt-8 Duster");
					fake.Pos = self.Pos + Vector(2 * self.FlipFactor, 0):RadRotate(self.RotAngle);
	...
	if self.FiredFrame then
		...
		self.angVel = self.angVel - RangeRand(0.7,1.1) * 8
	...
	if self.firingAnim == true and (not self.backFrame) then
		...
		self.Frame = 0 + math.floor((1 - factor) * 3 + 0.5)
