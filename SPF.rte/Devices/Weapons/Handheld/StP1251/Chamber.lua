...
function Create(self)
	...
	-- Sounds --
	self.grenadeRemoveSound = CreateSoundContainer("GrenadeRemove StP1 251", "SPF.rte");
	self.reflectionOutdoorsSound = CreateSoundContainer("ReflectionOutdoors StP1 251", "SPF.rte");
	
	self.reloadPrepareSounds = {}
	self.reloadPrepareSounds.RoundIn = CreateSoundContainer("RoundInPrepare StP1 251", "SPF.rte");
	self.reloadPrepareSounds.BoltBack = nil;
	
	self.reloadPrepareLengths = {}
	self.reloadPrepareLengths.RoundIn = 260
	self.reloadPrepareLengths.BoltBack = 0
	
	self.reloadPrepareDelay = {}
	self.reloadPrepareDelay.RoundIn = 600
	self.reloadPrepareDelay.BoltBack = 700
	
	self.reloadAfterSounds = {}
	self.reloadAfterSounds.RoundIn = CreateSoundContainer("RoundIn StP1 251", "SPF.rte");
	self.reloadAfterSounds.BoltBack = CreateSoundContainer("BoltBack StP1 251", "SPF.rte");
	
	self.reloadAfterDelay = {}
	self.reloadAfterDelay.RoundIn = 0
	self.reloadAfterDelay.BoltBack = 800
	...
	self.recoilStrength = 24 -- multiplier for base recoil added to the self.recoilStr when firing
	...
	if self.parent and self:IsReloading() then
		...
		if not self.spentRound then
			...
			if self:NumberValueExists("Switched") then
				self.grenadeRemoveSound:Play(self.Pos);
				if self:NumberValueExists("Frag Round") then
					local shell = CreateMOSRotating("StP1 251 Inert Grenade B");
					shell.Pos = self.MuzzlePos;
					shell.Vel = Vector(2*self.FlipFactor, 0):RadRotate(self.RotAngle);
					shell.HFlipped = self.HFlipped;
					MovableMan:AddParticle(shell);
				else
					local shell = CreateMOSRotating("StP1 251 Inert Grenade A");
					shell.Pos = self.MuzzlePos;
					shell.Vel = Vector(2*self.FlipFactor, 0):RadRotate(self.RotAngle);
					shell.HFlipped = self.HFlipped;
					MovableMan:AddParticle(shell);						
